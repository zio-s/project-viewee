...
const ReviewSection = ({ isOpen, onClose, reviews, setReviews }) => {
  ...
  const [editingIndex, setEditingIndex] = useState(null); // 현재 수정 중인 리뷰의 index

  useEffect(() => {
    const storedReviews = localStorage.getItem("reviews");
    if (storedReviews) {
      const parsedReviews = JSON.parse(storedReviews);
      console.log("Loaded reviews:", parsedReviews); // 추가된 콘솔 로그
    ...
    if (reviewText.trim()) {
      ...
      if (editingIndex !== null) {
        ...
        const updatedReviews = reviews.map((review, index) =>
          index === editingIndex
            ? { ...review, stars: selectedStars, text: reviewText, date: currentDate }
            : review
        );
  ...
  const handleEdit = (index) => {
    ...
    setEditingIndex(index); // 현재 수정 중인 리뷰의 index 설정
  ...
  return (
    <ModalOverlay>
      <ModalContent>
        ...
        />
        <SubmitButton onClick={handleSubmit} type="submit"> {editingIndex !== null ? "수정 완료" : "등록"}</SubmitButton>
